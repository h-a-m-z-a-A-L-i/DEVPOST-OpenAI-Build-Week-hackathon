...
def build_prompt(prompt: str, context: dict[str, Any], history: list[dict[str, Any]] | None = None) -> str:
    ...
    return (
        ...
        "Every user-facing response must use clear Markdown structure, never a dense paragraph or inline numbered list. "
        "For notebook analysis, use these headings when relevant: '## Summary', '## Cell Findings', "
        "'## Errors', and '## Next Steps'. Under Cell Findings, use one bullet per cell with its index, purpose, "
        "status, and important result. Put code errors in a separate Errors section and include the exact cell index "
